...
class Video_Caption_Generator():
    ...
    def build_model(self):
        ...
        state1 = (tf.zeros([self.batch_size,self.dim_hidden]),)*2
        state2 = (tf.zeros([self.batch_size,self.dim_hidden]),)*2
        padding = tf.zeros([self.batch_size,self.dim_hidden])
        ...
        for i in range(self.n_lstm_steps): ## Phase 2 => only generate captions
            ...
            cross_entropy = tf.nn.softmax_cross_entropy_with_logits_v2(labels=onehot_labels, logits=logit_words)
...
video_path = root_path+'data/youtube_videos'
video_data_path=root_path+'data/video_corpus.csv'
video_feat_path = root_path+'data/youtube_feats'

vgg16_path = root_path + 'caffe/models/tensorflow_vgg16/vgg16.tfmodel'

model_path = root_path + 'models_4_1/'
############## Train Parameters #################
dim_image = 4096
dim_hidden= 256
n_frame_step = 80
...
def train():
    train_data, _ = get_video_data(video_data_path, video_feat_path, train_ratio=0.9) #0.9
    ...
    np.save(root_path+'data/ixtoword', ixtoword)
    ...
    # train_op = tf.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(loss=tf_loss)
...
def test(model_path=model_path+'model-200', video_feat_path=video_feat_path):

    train_data, test_data = get_video_data(video_data_path, video_feat_path, train_ratio=0.9)
    test_videos = test_data['video_path'].unique()
    ixtoword = pd.Series(np.load(root_path+'data/ixtoword.npy').tolist())
...
if __name__=="__main__":
    # train()
    test(model_path = model_path+ 'model-200')
    # test(model_path+'model-900')
